    ...
    <PortalShell role="Teacher Portal" navItems={TEACHER_NAV} title="Teacher Faculty Profile">
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm mb-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5 border-b border-slate-100 pb-6 mb-6">
          ...
          <div>
            <h2 className="font-display text-xl font-bold text-slate-900">{teacher.user.name}</h2>
            <p className="text-xs text-slate-500 mt-0.5">
              Faculty Code: <span className="font-mono font-bold text-aec-navy">{teacher.teacherCode}</span>
            </p>
            <p className="text-xs text-slate-600 mt-1">
              Active Instructor • Appointed:{" "}
              {new Date(teacher.hiredAt).toLocaleDateString("en-US", {
                month: "long",
                year: "numeric"
              })}
            </p>
        ...
        <div className="grid gap-6 sm:grid-cols-2 text-xs">
          <div className="space-y-4">
            ...
            <div>
              ...
              <span className="font-medium text-slate-800 text-sm">{teacher.user.role}</span>
            </div>
          </div>
